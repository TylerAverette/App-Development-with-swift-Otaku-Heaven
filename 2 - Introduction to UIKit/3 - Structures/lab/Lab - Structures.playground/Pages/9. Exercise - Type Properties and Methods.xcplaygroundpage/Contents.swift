...
struct User {
    ...
    var email: String
    var age: Int
    static var nullUser: User = User(userName: "init", email: "", age: 0)
    static var currentUser: User = nullUser

    static func logIn(who: User){
        currentUser = who
        nullUser = User(userName: "Tyler", email: "", age: 0)
        print("\(currentUser) has logged in.")
    }
    static func logOut(){
        currentUser = nullUser
        print("\(nullUser.userName) has logged out")
        nullUser.userName = ""
    }
}

//let activeUser = User(userName: "Tyler", email: "example@example.com", age: 24, currentUser: "Tyler")
var activeUser: User = User(userName: "Tyler", email: "example@example.com", age: 24)
 ...
 */
User.logIn(who: User(userName: "Tyler", email: "user@gmail.com", age: 32))
...
/*:
 ...
 Below, call the `logIn(user:)` method and pass in a different `User` instance than what you assigned to currentUser above. Observe the printout in the console.
 */
activeUser = User(userName: "Tyler 2.0", email: "second@gmail.com", age: 40)
User.logIn(who: activeUser)
